...
if "all_cards" not in st.session_state:
    st.title("🃏 Flashcards — IÉSEG Vocabulaire")
    st.info("👈 Choisis une liste dans la barre latérale pour commencer !")

    st.markdown("---")
    st.markdown("### Comment ça marche ?")
    st.markdown("""
1. **Lis le mot** affiché en français
2. **Réfléchis** à la traduction allemande
3. **Révèle la réponse** et évalue-toi honnêtement :
   - ✅ **Je sais** — tu l'avais du premier coup, ou tu te souvenais du mot sans hésiter
   - 🟡 **À peu près** — tu hésitais, ou tu connaissais le mot mais pas l'article 😉
   - ❌ **Je ne sais pas** — la carte reviendra vite

**Le système travaille pour toi** — les mots que tu ne connais pas restent en rotation, les mots maîtrisés sortent et laissent place à de nouveaux. Tu travailles toujours sur un petit groupe de 10 cartes à la fois.
""")

    st.markdown("---")
    st.markdown("### 📚 Listes disponibles")
